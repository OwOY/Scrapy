...
class NynewsSpider(scrapy.Spider):
    ...
    start_urls = ["https://i.news.qq.com/trpc.qqnews_web.kv_srv.kv_srv_http_proxy/list?sub_srv_id=pet&srv_id=pc&offset=0&limit=20&strategy=1&ext={%22pool%22:[%22top%22],%22is_filter%22:7,%22check_type%22:true}"]
                    
    def parse(self, response):
        # yield scrapy.Request(f"{self.start_urls[0]}", callback=self.parse_article_urls) 
        offset = 0
        while offset < 200:
            next_urls = 'https://i.news.qq.com/trpc.qqnews_web.kv_srv.kv_srv_http_proxy/list?sub_srv_id=pet&srv_id=pc&offset='+str(offset)+'&limit=20&strategy=1&ext={%22pool%22:[%22top%22],%22is_filter%22:7,%22check_type%22:true}'
